...
def test_loadcell_init_success():
    with (
        patch("dica.hardware.loadcell.HX711_AVAILABLE", True),
...
def test_loadcell_init_fail():
    with (
        ...
        patch("dica.hardware.loadcell.board"),
        patch(
            "dica.hardware.loadcell.digitalio.DigitalInOut", side_effect=Exception("Hardware Error")
        ),
    ):
        lc = LoadCell()
        lc.init_loadcell()
...
def test_loadcell_init_prod_error(monkeypatch):
    ...
    from dica.hardware import loadcell
