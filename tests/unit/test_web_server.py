...
def test_api_validate_valid_data(client):
    ...
    payload = {
        'items': [
            {'name': 'nasi_porsi', 'price': 5000},
            {'name': 'telur', 'price': 3000}
        ]
    }
    response = cli.post('/api/validate', json=payload, headers={'Authorization': f'Bearer {config.API_KEY}'})
    assert response.status_code == 200
...
def test_api_clear_transactions(client):
    cli, mock_main = client
    response = cli.post('/api/clear_transactions', headers={'Authorization': f'Bearer {config.API_KEY}'})
    assert response.status_code == 200
...
def test_api_sync_cloud(client):
    ...
    with patch('dica.api.web_server.CloudSync') as mock_cloud:
        ...
        response = cli.post('/api/sync_cloud', headers={'Authorization': f'Bearer {config.API_KEY}'})
        assert response.status_code == 200
        data = json.loads(response.data)
