...
def test_printer_init_success():
    with patch('dica.hardware.printer.PRINTER_AVAILABLE', True):
        with patch('dica.hardware.printer.Usb', create=True) as mock_usb:
            printer = ReceiptPrinter()
...
def test_printer_print_receipt_success():
    with patch('dica.hardware.printer.PRINTER_AVAILABLE', True):
        with patch('dica.hardware.printer.Usb', create=True):
            printer = ReceiptPrinter()
