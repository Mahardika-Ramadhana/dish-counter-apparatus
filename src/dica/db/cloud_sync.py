...
class CloudSync:
    def __init__(
        self, db_lokal: str = "transaksi.db", supabase_url: str = "", supabase_key: str = ""
    ):
    ...
    def sync_unpushed_transactions(self) -> dict[str, Any]:
        ...
        try:
            ...
            res = (
                self.client.table("transaksi_cloud")
                .upsert(payload, on_conflict="id_lokal")
                .execute()
            )
            ...
            return {"status": "error", "message": f"Gagal sinkronisasi: {e!s}"}
