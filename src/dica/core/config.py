...
def load_json_config():
    ...
    if not os.path.exists(_CONFIG_FILE):
        raise FileNotFoundError(f"❌ ERROR: File {_CONFIG_FILE} TIDAK DITEMUKAN! Mesin kasir menolak menyala sebelum daftar harga diisi.")
    ...
    try:
        ...
        raise ValueError(f"❌ ERROR: Format config.json rusak! Tolong perbaiki penulisan JSON-nya. Detail: {e}")

_json_data = load_json_config()

# Cek apakah bagian 'harga' ada di dalam JSON
if 'harga' not in _json_data:
    raise ValueError("❌ ERROR: DAFTAR HARGA KOSONG! Mesin kasir menolak menyala. Tolong isi bagian 'harga' di config.json.")
HARGA = _json_data['harga']

# Cek apakah bagian 'berat_rata_lauk' ada di dalam JSON
if 'berat_rata_lauk' not in _json_data:
    raise ValueError("❌ ERROR: DATA BERAT LAUK KOSONG! Tolong isi bagian 'berat_rata_lauk' di config.json.")
...
if 'berat_satu_porsi_nasi' not in _json_data:
    raise ValueError("❌ ERROR: BERAT PORSI NASI KOSONG! Tolong isi bagian 'berat_satu_porsi_nasi' di config.json.")
...
if 'berat_satu_porsi_sambal' not in _json_data:
    raise ValueError("❌ ERROR: BERAT PORSI SAMBAL KOSONG! Sambal adalah barang tak terhitung, tolong isi 'berat_satu_porsi_sambal' di config.json.")
