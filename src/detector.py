    ...
    def load_model(self):
        if not YOLO_AVAILABLE:
            ...
            try:
                ...
                mb.showerror("Error AI", f"Ultralytics gagal dimuat di Raspberry Pi!\n\nAlasan: {e}\n\nKemungkinan proses instalasi belum selesai atau gagal karena Raspberry Pi kehabisan memori.")
                
        if YOLO_AVAILABLE:
            print("INFO: Menggunakan model kustom YOLOv8 (2 Kelas: Ayam, Nasi) untuk Uji Coba.")
            # Nanti ubah ke file .tflite jika konversi sudah selesai dan ingin performa Raspberry Pi yang ngebut
            self.yolo_model = YOLO('models/dica_2kelas.pt', task='segment')
            self.is_dummy = False
            return
    ...
    def detect(self, frame: np.ndarray) -> List[Dict[str, Any]]:
        ...
        if self.yolo_model:
            try:
                ...
                if len(results) > 0:
                    ...
                    if r.masks is not None and r.boxes is not None:
                        for i in range(len(r.boxes)):
                            # Dapatkan ID Kelas Langsung dari AI (Tanpa hack RGB lagi!)
                            cls_id = int(r.boxes.cls[i])
                            # Di data.yaml: 0 = Ayam Goreng, 1 = Nasi
                    ...
                    for det in detections:
                        ...
                        cv2.rectangle(self.last_annotated_frame, (x1, max(0, y1 - 25)), (x1 + tw + 10, max(0, y1)), (0, 0, 0), -1)
                        # Tulis teks Nasi/Ayam
                        cv2.putText(self.last_annotated_frame, label, (x1 + 5, max(0, y1 - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
                ...
                cv2.putText(err_frame, "AI ERROR: " + error_msg, (20, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
